...
Scraper::Parser - class to manage campaign data parsing
...
sub validate_not_found {
	...
	my $data = shift
		or die __PACKAGE__ . "->validate_not_found() missing argument";

	if ( $self->logging ) {

		$self->{logging}->entry( 2, "Validateing 404 response data ..." );
	}
...
sub parse {
	...
	if ( $self->type ) {
		...
		if ( $self->{type} eq qw(scrape) ) {
			...
			die __PACKAGE__ . "->parse() unknown parser type";
		}
	...
	die __PACKAGE__ . "->parse() undefined parser type";
}
